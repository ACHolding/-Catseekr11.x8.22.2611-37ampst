#!/usr/bin/env python3
"""catseekr1-10b — RAM-only BitNet b1.58 ~10B on DeepSeek V4 Flash (files=off).

catseekr1-10b materializes a real ~10 billion-parameter BitNet b1.58 Transformer
in RAM when the program starts (files=off). Boot target ≈ **0.2 seconds** via a
sparse 2-bit bank (resident STE pages only; virtual ~10B slots). The inference
trunk is **DeepSeek V4 Flash**: Compressed Sparse Attention (CSA), Heavily
Compressed Attention (HCA), Manifold-Constrained Hyper-Connections (mHC), and
Hash-MoE → DeepSeekMoE — all executed through real BitLinear ternary kernels
with STE training on the resident shadow slice.
Tokenizer, bilingual routing, vibe-check corpus, next-token softmax, 60 FPS
GUI/CLI, Code, Build, and self-tests live in this file.

Real BitNet (Microsoft BitNet b1.58) at ~10B × DeepSeek V4 Flash:
    ...
    * ~10B ternary slots; sparse pages materialize the live STE slice only
    * resident FP32 shadows train with STE; packed bank stays files=off
    * DeepSeekV4FlashBackend (files=off): concat sliding+compressor KV, sinks, mHC streams
    * V4 Flash hybrid attention: sliding-window bootstrap, CSA (m=4 + indexer), HCA
    * mHC mixes residual streams with Sinkhorn doubly-stochastic maps
    * Hash-MoE tid2eid table in RAM; later layers use sqrt-softplus top-k
    * MTP-style depth mix + DSpark speculative decode — files=off
    * DeepSeek R1-style test-time reasoning (think/answer + GRPO reward) — files=off
    * no dual-residual fake ternary; no downloaded checkpoint
    * default boot_budget_s ≈ 0.2; STE warmup deferred to ``/train``
    * files=off also means **no generated programs on disk** — the only program is this ``.py``
...
files = off:
    ...
    * no apps, games, or scripts are emitted (no ``builds/``, no Write/Edit/Bash)
    * Code/Build answer in text; the runtime is ``catsseekr1.xx.py`` itself

This is an educational local 10B-slot BitNet with V4 Flash *mechanisms*,
not the 284B DeepSeek-V4-Flash pretrained checkpoint.

Launch ``python3 catsseekr1.xx.py`` — the model, tokenizer, and replies stay in this file.
"""
...
try:
    ...
    raise SystemExit(
        "catseekr1-10b requires NumPy for real in-memory BitNet training. "
        "No model checkpoint is required."
    ) from exc


APP_NAME = "catseekr1-10b"
APP_VERSION = "10B.1"
MODEL_ID = "catseekr1-10b-bitnet-b1.58-deepseek-v4-flash-dspark-ram"
FILES_MODE = "off"  # no checkpoints, no downloads, no generated programs — RAM + this .py only
SCRIPT_NAME = "catsseekr1.xx.py"


def files_off_no_programs(*, kind: str = "program") -> str:
    """Refuse disk artifacts: the only program is this .py (files=off)."""
    return (
        f"files=off: I do not generate {kind} files. "
        f"The only program is `{SCRIPT_NAME}` — BitNet, tokenizer, and replies live inside that one file. "
        "Nothing is written to disk (no builds/, no new scripts)."
    )
ENGINE_ID = "deepseek-v4-flash"
ENGINE_NAME = "DeepSeek V4 Flash"
GUI_FPS = 60
GUI_TICK_MS = max(1, 1000 // GUI_FPS)
DEFAULT_SEED = 0xCA75_EE41
BITNET_QB = 127  # INT8 absmax range for BitNet A8 activations
BITNET_EPS = 1e-5
# Deep-narrow BitNet Transformer ≈ 10.0B ternary weights (files=off, packed).
# 3180 × (4×512² + 2×512×2048) = 3180 × 3,145,728 = 10,003,415,040
BITNET_10B_N_LAYERS = 3180
BITNET_10B_D_MODEL = 512
BITNET_10B_D_FF = 2048
BITNET_10B_N_HEADS = 8
# DeepSeek V4 Flash hybrid attention / MoE (educational scale, files=off).
DEEPSEEK_V4_CSA_COMPRESS = 4
DEEPSEEK_V4_HCA_COMPRESS = 8
DEEPSEEK_V4_SLIDING_WINDOW = 16
DEEPSEEK_V4_INDEX_TOPK = 2
DEEPSEEK_V4_HC_MULT = 4
DEEPSEEK_V4_HASH_MOE_LAYERS = 3
DEEPSEEK_MOE_TOP_K = 2
...
class ModelConfig:
    ...
    embedding_dim: int = BITNET_10B_D_MODEL
    hidden_dim: int = BITNET_10B_D_FF
    d_model: int = BITNET_10B_D_MODEL
    d_ff: int = BITNET_10B_D_FF
    n_layers: int = BITNET_10B_N_LAYERS
    n_heads: int = BITNET_10B_N_HEADS
    mod_capacity: int = 2
    train_layers: int = 1
    ...
    csa_compress: int = DEEPSEEK_V4_CSA_COMPRESS
    hca_compress: int = DEEPSEEK_V4_HCA_COMPRESS
    sliding_window: int = DEEPSEEK_V4_SLIDING_WINDOW
    index_topk: int = DEEPSEEK_V4_INDEX_TOPK
    hc_mult: int = DEEPSEEK_V4_HC_MULT
    hash_moe_layers: int = DEEPSEEK_V4_HASH_MOE_LAYERS
    moe_top_k: int = DEEPSEEK_MOE_TOP_K
...
# ──────────────────────────────────────────────────────────────
# DSPARK × BITNET + DEEPSEEK V4 FLASH (files = off · speculative decode)
...
class DSparkBitNetEngine:
    """DSpark speculative decode on the BitNet + DeepSeek V4 Flash trunk (files=off).
...
CODE_LINE_RE = re.compile(
    r"^[ \t]*(?:def |async def |class |function |fun |fn |pub |let |const |var |"
    r"import |from |package |#include|using |template |elif |except |"
    r"return |print\(|console\.|if \(|for \(|while \()",
    re.MULTILINE,
)
...
def looks_like_code(text: str) -> bool:
    """Heuristic: fenced blocks, language keywords, or symbol-dense source."""
    raw = str(text or "")
    if CODE_FENCE_RE.search(raw):
        return True
    if re.search(
        r"\b(?:python|javascript|typescript|golang|rustc|\.py\b|\.js\b|\.ts\b|\.rs\b|\.go\b)\b",
        raw,
        re.I,
    ) and re.search(r"[(){};=]|def |class |function |fn |import ", raw, re.I):
        return True
    code_lines = sum(1 for line in raw.splitlines() if CODE_LINE_RE.search(line))
    if code_lines >= 2:
        return True
    if CODE_LINE_RE.search(raw) and ("(" in raw or ":" in raw):
        return True
    symbols = len(CODE_TOKEN_RE.findall(raw))
    letters = max(1, len(LATIN_RE.findall(raw)) + len(HAN_RE.findall(raw)))
    return symbols >= 10 and symbols / letters > 0.14


def detect_modality(text: str) -> str:
    """Return ``code``, ``zh``, ``en``, or ``mixed`` (code + Mandarin)."""
    code = looks_like_code(text)
    language = detect_language(text)
    if code and language == "zh":
        return "mixed"
    if code:
        return "code"
    return language
...
BOOT_DIALOGUES: tuple[tuple[tuple[str, ...], str], ...] = (
    (("hello", "hi", "hey catseekr1-10b", "good morning"),
     "Hello! I am catseekr1-10b, running locally from RAM. What are we building?"),
    (("who are you", "what model are you", "tell me about yourself"),
     "I am catseekr1-10b, a compact next-token language model trained in memory when this program starts."),
    ...
    (("what engine do you use", "what is deepseek v4 flash", "deepseek v4 flash", "v4 flash", "deepseek flash"),
     "My inference trunk is DeepSeek V4 Flash on real BitNet: CSA, HCA, mHC, and Hash-MoE / DeepSeekMoE, all files=off in RAM."),
    ...
    (("what is dual residual ternary", "explain your new math", "what is dual residual ternary"),
     "Earlier builds used a dual-residual ternary shortcut. This build is real BitNet b1.58 BitLinear on DeepSeek V4 Flash: absmean ternary matrices, A8 activations, CSA/HCA/mHC/Hash-MoE, and an integer ternary matmul kernel."),
    ...
    (("how do your reasoning passes work", "explain latent reasoning", "do you use extra compute per token"),
     "Before predicting each token, DeepSeek V4 Flash Mixture-of-Depths selects a capacity-bounded BitNet layer set, mixes residual streams with mHC, and runs Hash-MoE / DeepSeekMoE experts."),
    ...
    (("what is tokenization", "explain tokens", "how do you tokenize text"),
     "Tokenization converts text into vocabulary identifiers. catseekr1-10b uses compact word, punctuation, newline, and control tokens."),
    (("write Python hello world", "make a Python hello program", "Python print example"),
     "Here is Python code:\n```\nprint(\"Hello from catseekr1-10b\")\n```"),
    ...
    (("make a game", "game development plan", "how do I build a game"),
     "files=off: I do not generate game or app files. The only program is catsseekr1.xx.py — ask in chat and I answer from the BitNet that lives in that file."),
    (("build an app", "make an app", "grok build", "catseek build"),
     "files=off: catseekr1-10b does not write programs to disk. Everything is inside catsseekr1.xx.py. Describe the idea in chat; I will not create builds/ or new scripts."),
    ...
    (("thank you", "thanks", "nice work"),
     "You are welcome! catseekr1-10b is ready for the next task."),
    (("goodbye", "bye", "see you later"),
     "Goodbye! The conversation can end while the in-memory model remains ready."),
    (("你好", "嗨", "早上好", "晚上好"),
     "你好！我是 catseekr1-10b。检测到中文后，我会自动使用中文回答。"),
    (("你是谁", "你是什么模型", "请介绍一下自己"),
     "我是 catseekr1-10b，一个启动时在内存中训练的本地下一词元语言模型。"),
    ...
    (("什么是双残差三值量化", "解释你的新数学", "什么是双残差三值"),
     "旧版用双残差三值近似。本版是真正的 BitNet b1.58，推理引擎为 DeepSeek V4 Flash：每层 absmean 三值矩阵、A8 激活、CSA/HCA/mHC/Hash-MoE，以及整数加减三值乘加。"),
    ...
    (("什么是DeepSeek V4 Flash", "你用什么引擎"),
     "我的推理主干是挂在 BitNet 上的 DeepSeek V4 Flash：CSA、HCA、mHC 与 Hash-MoE，全部 files=off 驻留内存。"),
    (("你的推理循环怎样工作", "解释潜在推理", "每个词元会多次计算吗"),
     "预测每个词元前，DeepSeek V4 Flash 用 Mixture-of-Depths 选出容量内的 BitNet 层，经 mHC 混合残差流，再跑 Hash-MoE 专家。"),
    ...
    (("什么是分词", "解释词元", "文本怎样变成数字"),
     "分词把文字转换成词表编号。catseekr1-10b 对英文使用单词和标点，对中文使用汉字和标点。"),
...
# When catseekr1-10b lacks grounded knowledge, the correct learned behavior is to say
...
class TernaryBank:
    """Sparse 2-bit BitNet bank: virtual ~10B slots, resident STE pages only.

    Boot stays ≤0.2s because we never allocate the full ~2.5GB packed buffer up front.
...
class DeepSeekV4FlashBackend:
    """DeepSeek V4 Flash inference backend (files=off, RAM-only).

    Paper mechanisms implemented on BitLinear kernels — no checkpoint, no download:
      * layer_types: sliding-window / CSA / HCA
      * concatenated sliding KV + compressor KV with block_bias mask
      * shared K=V MQA, partial RoPE, per-head attention sinks
      * grouped output projection through BitLinear W_o
      * mHC streams with Sinkhorn doubly-stochastic maps
      * Hash-MoE tid2eid table synthesized in RAM + sqrt-softplus routed MoE
      * MTP depth mix
    """

    __slots__ = (
        "lm", "tid2eid", "attn_sinks", "mhc_attn", "mhc_ffn", "mtp_queries",
        "moe_router", "moe_bias", "o_groups",
        "csa_calls", "hca_calls", "flash_attn_calls", "mhc_mixes",
        "mtp_mixes", "moe_routes", "hash_moe_hits", "last_path",
    )

    def __init__(self, lm: "InMemoryTernaryLM"):
        if FILES_MODE != "off":
            raise RuntimeError("DeepSeek V4 Flash backend requires files=off")
        self.lm = lm
        cfg = lm.config
        rng = lm.rng
        vocab = max(2, lm.tokenizer.vocab_size)
        n_exp = max(2, int(cfg.moe_top_k) + 2)
        # Frozen hash table — synthesized in RAM, never read from a V4 checkpoint.
        self.tid2eid = ((np.arange(vocab, dtype=np.int64) * np.int64(2654435761) + np.int64(cfg.seed)) % n_exp).astype(np.int32)
        n_heads = max(1, int(cfg.n_heads))
        self.attn_sinks = rng.normal(0.0, 0.02, size=(n_heads,)).astype(np.float32)
        hc = max(2, int(cfg.hc_mult))
        self.mhc_attn = rng.normal(0.0, 0.08, size=(8, hc, hc)).astype(np.float32)
        self.mhc_ffn = rng.normal(0.0, 0.08, size=(8, hc, hc)).astype(np.float32)
        self.mtp_queries = rng.normal(0.0, 0.02, size=(max(2, cfg.mod_capacity + 2), cfg.d_model)).astype(np.float32)
        self.moe_router = rng.normal(0.0, 0.05, size=(cfg.d_model, n_exp)).astype(np.float32)
        self.moe_bias = rng.normal(0.0, 0.02, size=(n_exp,)).astype(np.float32)
        self.o_groups = max(1, min(int(getattr(cfg, "o_groups", 4)), n_heads))
        self.csa_calls = 0
        self.hca_calls = 0
        self.flash_attn_calls = 0
        self.mhc_mixes = 0
        self.mtp_mixes = 0
        self.moe_routes = 0
        self.hash_moe_hits = 0
        self.last_path = "sliding"

    def layer_type(self, layer_index: int) -> str:
        if layer_index < 2 and self.lm.train_layer_count > 2:
            return "sliding"
        return "csa" if (layer_index % 2) == 0 else "hca"

    def mlp_type(self, layer_index: int) -> str:
        return "hash_moe" if layer_index < max(1, int(self.lm.config.hash_moe_layers)) else "moe"

    def attention(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        hidden = self.lm.rms_norm(hidden)
        if not inference:
            self.last_path = "sliding-train"
            self.lm.last_engine_path = self.last_path
            return self._sliding(hidden, layer_index, inference=False)
        kind = self.layer_type(layer_index)
        self.last_path = kind
        self.lm.last_engine_path = kind
        if kind == "hca":
            out = self._hca(hidden, layer_index, inference=True)
            self.hca_calls += 1
        elif kind == "csa":
            out = self._csa(hidden, layer_index, inference=True)
            self.csa_calls += 1
        else:
            out = self._sliding(hidden, layer_index, inference=True)
        self.flash_attn_calls += 1
        self.lm.flash_attn_calls = self.flash_attn_calls
        self.lm.csa_calls = self.csa_calls
        self.lm.hca_calls = self.hca_calls
        return out

    def _project_qkv(self, hidden: np.ndarray, layer_index: int, *, inference: bool):
        lm = self.lm
        batch, seq, d_model = hidden.shape
        n_heads = lm.config.n_heads
        head_dim = d_model // n_heads
        flat = hidden.reshape(batch * seq, d_model)
        q = lm._bitlinear(flat, layer_index, 0, inference=inference)
        k = lm._bitlinear(flat, layer_index, 1, inference=inference)
        v = lm._bitlinear(flat, layer_index, 2, inference=inference)
        # Shared K=V MQA (V4 n_kv_heads=1): average K/V, collapse heads, broadcast.
        kv = ((k + v) * 0.5).reshape(batch, seq, n_heads, head_dim).mean(axis=2, keepdims=True)
        q = q.reshape(batch, seq, n_heads, head_dim).transpose(0, 2, 1, 3)
        k = np.broadcast_to(kv.transpose(0, 2, 1, 3), q.shape).copy()
        v = k.copy()
        q, k = lm._partial_rope(q, k)
        return q, k, v, batch, seq, d_model

    def _sliding_mask(self, seq: int) -> np.ndarray:
        window = max(1, min(int(self.lm.config.sliding_window), seq))
    ...
    def _attend(self, q: np.ndarray, k: np.ndarray, v: np.ndarray, mask: np.ndarray) -> np.ndarray:
        scale = 1.0 / math.sqrt(max(1, q.shape[-1]))
        attn = np.matmul(q, np.transpose(k, (0, 1, 3, 2))) * scale
        attn = attn + mask.astype(np.float32)
        # Per-head attention sink (V4 eq. 27): extra logit, no extra value mass.
        sink = self.attn_sinks[None, : q.shape[1], None, None]
        attn = np.concatenate([attn, np.broadcast_to(sink, attn.shape[:3] + (1,))], axis=-1)
        ...
        core = weights[..., :-1]
        return np.matmul(core, v)

    def _pool_kv(self, k: np.ndarray, v: np.ndarray, rate: int, *, overlap: bool):
        rate = max(1, int(rate))
        seq = k.shape[2]
        step = max(1, rate // 2) if overlap else rate
        starts = list(range(0, seq, step))
        if not starts:
            return k[:, :, :1, :], v[:, :, :1, :], [0]
        pooled_k, pooled_v, closes = [], [], []
        for start in starts:
            end = min(seq, start + rate)
            pooled_k.append(k[:, :, start:end, :].mean(axis=2, keepdims=True))
            pooled_v.append(v[:, :, start:end, :].mean(axis=2, keepdims=True))
            closes.append(end - 1)
        return np.concatenate(pooled_k, axis=2), np.concatenate(pooled_v, axis=2), closes

    def _concat_mask(self, seq: int, n_pools: int, kind: str, q: np.ndarray, pool_k: np.ndarray, closes: list[int]) -> np.ndarray:
        """V4 layout: [sliding causal | compressor block_bias] along the key axis."""
        local = self._sliding_mask(seq)
        block = np.full((seq, n_pools), -1e9, dtype=np.float32)
        if kind == "hca":
            for query_t in range(seq):
                for pool_i, close_t in enumerate(closes):
                    if close_t <= query_t:
                        block[query_t, pool_i] = 0.0
        else:
            # CSA lightning indexer: top-k compressed entries per query.
            scale = 1.0 / math.sqrt(max(1, q.shape[-1]))
            scores = np.matmul(q, np.transpose(pool_k, (0, 1, 3, 2))) * scale
            scores = scores.mean(axis=(0, 1))  # T, P
            topk = max(1, min(int(self.lm.config.index_topk), n_pools))
            for query_t in range(seq):
                ready = [i for i, close_t in enumerate(closes) if close_t <= query_t]
                if not ready:
                    continue
                ranked = sorted(ready, key=lambda i: float(scores[query_t, i]), reverse=True)[:topk]
                for pool_i in ranked:
                    block[query_t, pool_i] = 0.0
        return np.concatenate([local, block], axis=-1)

    def _hybrid(self, hidden: np.ndarray, layer_index: int, *, inference: bool, kind: str) -> np.ndarray:
        q, k, v, batch, seq, d_model = self._project_qkv(hidden, layer_index, inference=inference)
        if kind == "sliding":
            mixed = self._attend(q, k, v, self._sliding_mask(seq))
        else:
            rate = self.lm.config.csa_compress if kind == "csa" else self.lm.config.hca_compress
            overlap = kind == "csa"
            pool_k, pool_v, closes = self._pool_kv(k, v, rate, overlap=overlap)
            k_cat = np.concatenate([k, pool_k], axis=2)
            v_cat = np.concatenate([v, pool_v], axis=2)
            mask = self._concat_mask(seq, pool_k.shape[2], kind, q, pool_k, closes)
            mixed = self._attend(q, k_cat, v_cat, mask)
        return self._grouped_o_proj(mixed, layer_index, inference=inference)

    def _grouped_o_proj(self, mixed_heads: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        """V4 grouped output projection: low-rank mix inside o_groups, then BitLinear W_o."""
        batch, n_heads, seq, head_dim = mixed_heads.shape
        d_model = n_heads * head_dim
        groups = max(1, min(self.o_groups, n_heads))
        mixed_bt = mixed_heads.transpose(0, 2, 1, 3)
        hpg = max(1, n_heads // groups)
        pieces = []
        for gi in range(groups):
            h0 = gi * hpg
            h1 = n_heads if gi == groups - 1 else (gi + 1) * hpg
            grp = mixed_bt[:, :, h0:h1, :]
            mean = grp.mean(axis=2, keepdims=True)
            pieces.append((0.85 * grp + 0.15 * mean).astype(np.float32))
        fused = np.concatenate(pieces, axis=2).reshape(batch * seq, d_model)
        out = self.lm._bitlinear(fused, layer_index, 3, inference=inference)
        return out.reshape(batch, seq, d_model)

    def _sliding(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self._hybrid(hidden, layer_index, inference=inference, kind="sliding")

    def _csa(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self._hybrid(hidden, layer_index, inference=inference, kind="csa")

    def _hca(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self._hybrid(hidden, layer_index, inference=inference, kind="hca")

    def ffn(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        lm = self.lm
        hidden = lm.rms_norm(hidden)
        batch, seq, d_model = hidden.shape
        flat = hidden.reshape(batch * seq, d_model)
        shared = lm._ffn_expert(flat, layer_index, inference=inference)
        self.moe_routes += 1
        lm.moe_routes = self.moe_routes
        if (not inference) or lm.config.moe_top_k <= 1 or len(lm._live_layers()) <= 1:
            return shared.reshape(batch, seq, d_model)
        live = lm._live_layers()
        top_k = max(1, min(int(lm.config.moe_top_k), len(live)))
        probe = np.mean(flat, axis=0)
        scores = []
        for expert_index in live:
            affinity = float(probe @ self.moe_router[:, expert_index % self.moe_router.shape[1]])
            affinity += float(self.moe_bias[expert_index % self.moe_bias.shape[0]])
            if expert_index == layer_index:
                affinity += 1.25
            score = float(lm._sqrt_softplus(np.asarray([affinity], dtype=np.float32))[0])
            scores.append((score, expert_index))
        if self.mlp_type(layer_index) == "hash_moe" and lm._fwd_token_ids:
            token = int(lm._fwd_token_ids[-1]) % self.tid2eid.shape[0]
            hashed = int(self.tid2eid[token]) % len(live)
            chosen = [live[hashed]]
            for _score, expert_index in sorted(scores, reverse=True):
                if expert_index not in chosen:
                    chosen.append(expert_index)
                if len(chosen) >= top_k:
                    break
            self.hash_moe_hits += 1
            lm.hash_moe_hits = self.hash_moe_hits
            lm.last_engine_path = f"{self.last_path}+hash-moe"
        else:
        ...
        raw = np.asarray([float(score_map.get(idx, 0.05)) for idx in chosen], dtype=np.float32)
        weights = raw / np.maximum(np.sum(raw), BITNET_EPS)
        routed = np.zeros_like(shared)
        for weight, expert_index in zip(weights, chosen):
            if expert_index == layer_index:
                routed += weight * shared
            else:
                routed += weight * lm._ffn_expert(flat, expert_index, inference=inference)
        mix = (shared + 1.5 * routed).astype(np.float32)
        return lm.rms_norm(mix).reshape(batch, seq, d_model)

    @staticmethod
    def sinkhorn(matrix: np.ndarray, iters: int = 8, eps: float = 1e-6) -> np.ndarray:
        x = np.abs(matrix.astype(np.float32)) + eps
        for _ in range(max(1, iters)):
            x = x / np.maximum(x.sum(axis=1, keepdims=True), eps)
            x = x / np.maximum(x.sum(axis=0, keepdims=True), eps)
        return x.astype(np.float32)

    def mhc(self, residual: np.ndarray, sublayer: np.ndarray, which: str, layer_index: int) -> np.ndarray:
        if not self.lm.config.mhc_enabled:
            return (residual + sublayer).astype(np.float32)
        n = max(2, int(self.lm.config.hc_mult))
        table = self.mhc_attn if which == "attn" else self.mhc_ffn
        raw = table[layer_index % table.shape[0]]
        iters = max(1, int(getattr(self.lm.config, "hc_sinkhorn_iters", 8)))
        mix = self.sinkhorn(raw, iters=iters)
        streams = np.stack(
            [residual, sublayer, self.lm.rms_norm(residual), np.tanh(np.clip(sublayer, -8.0, 8.0))],
            axis=-2,
        )
        if streams.shape[-2] < n:
            pad = np.repeat(streams[..., -1:, :], n - streams.shape[-2], axis=-2)
            streams = np.concatenate([streams, pad], axis=-2)
        streams = streams[..., :n, :]
        mixed = np.einsum("...nd,mn->...md", streams, mix)
        self.mhc_mixes += 1
        self.lm.mhc_mixes = self.mhc_mixes
        return mixed.mean(axis=-2).astype(np.float32)

    def mtp_combine(self, snapshots: list[np.ndarray]) -> np.ndarray:
        if (not self.lm.config.mhc_enabled) or len(snapshots) == 1:
            return snapshots[-1]
        stack = np.stack(snapshots, axis=1)
        depth = stack.shape[1]
        query = self.mtp_queries[min(depth - 1, self.mtp_queries.shape[0] - 1)]
        ...
        self.mtp_mixes += 1
        self.lm.mtp_mixes = self.mtp_mixes
        return mixed

    def block(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        residual = hidden
        attn = self.attention(hidden, layer_index, inference=inference)
        hidden = self.mhc(residual, attn, "attn", layer_index)
        hidden = np.nan_to_num(hidden, nan=0.0, posinf=8.0, neginf=-8.0)
        residual = hidden
        ffn = self.ffn(hidden, layer_index, inference=inference)
        hidden = self.mhc(residual, ffn, "ffn", layer_index)
    ...
    def status(self) -> dict[str, object]:
        return {
            ...
            "files": FILES_MODE,
            "checkpoint_bytes": 0,
            "tid2eid_ram_slots": int(self.tid2eid.size),
            "hash_moe_hits": self.hash_moe_hits,
            "csa_calls": self.csa_calls,
            "hca_calls": self.hca_calls,
            "mhc_mixes": self.mhc_mixes,
            "mtp_mixes": self.mtp_mixes,
            "moe_routes": self.moe_routes,
            "last_path": self.last_path,
            "o_groups": self.o_groups,
            "layer_types": ["sliding", "compressed_sparse_attention", "heavily_compressed_attention"],
            "mlp_layer_types": ["hash_moe", "moe"],
        }


class InMemoryTernaryLM:
    """Real BitNet b1.58 ~10B LLM on DeepSeek V4 Flash (W1.58A8), files=off.

    Default topology lands at ~10.0B ternary slots:
      n_layers=3180, d_model=512, d_ff=2048, n_heads=8
      ...
      DeepSeek V4 Flash: CSA + HCA + sliding window, mHC, Hash-MoE
    ...
    def __init__(self, tokenizer: WordTokenizer, config: ModelConfig):
        ...
        self.last_engine_path: str = "sliding"
        self._fwd_token_ids: list[int] = []
        ...
        # DeepSeek V4 Flash tables live on DeepSeekV4FlashBackend (wired after the bank).
        ...
        self.v4 = DeepSeekV4FlashBackend(self)
        self.mhc_attn = self.v4.mhc_attn
        self.mhc_ffn = self.v4.mhc_ffn
        self.mtp_queries = self.v4.mtp_queries
        self.moe_router = self.v4.moe_router
        self.moe_bias = self.v4.moe_bias
    ...
    def rms_norm(values: np.ndarray, eps: float = BITNET_EPS) -> np.ndarray:
        """DeepSeek RMSNorm on the residual stream (BitLinear still uses LN)."""
        rms = np.sqrt(np.mean(values * values, axis=-1, keepdims=True) + eps)
        return (values / rms).astype(np.float32)
    ...
    def _attention(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self.v4.attention(hidden, layer_index, inference=inference)

    def _flash_attn_kind(self, layer_index: int) -> str:
        return self.v4.layer_type(layer_index)

    def _project_qkv(
        self,
        hidden: np.ndarray, layer_index: int, *, inference: bool,
    ) -> tuple[np.ndarray, np.ndarray, np.ndarray, int, int, int]:
        return self.v4._project_qkv(hidden, layer_index, inference=inference)

    def _partial_rope(self, q: np.ndarray, k: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """V4 Flash partial RoPE on the trailing half of each head."""
        seq = q.shape[2]
        head_dim = q.shape[-1]
        half = max(1, head_dim // 2)
        pos = np.arange(seq, dtype=np.float32)
        freq = 1.0 / (10000.0 ** (np.arange(half, dtype=np.float32) / float(half)))
        angles = pos[:, None] * freq[None, :]
        cos = np.cos(angles)[None, None, :, :]
        sin = np.sin(angles)[None, None, :, :]

        def rotate(x: np.ndarray) -> np.ndarray:
            lead = x[..., : x.shape[-1] - half]
            rot = x[..., x.shape[-1] - half :]
            width = min(rot.shape[-1] // 2, cos.shape[-1])
            if width < 1:
                return x.astype(np.float32)
            a, b = rot[..., :width], rot[..., width:width * 2]
            c, s = cos[..., :width], sin[..., :width]
            spun = np.concatenate([a * c - b * s, a * s + b * c], axis=-1)
            if rot.shape[-1] > spun.shape[-1]:
                spun = np.concatenate([spun, rot[..., spun.shape[-1]:]], axis=-1)
            return np.concatenate([lead, spun], axis=-1).astype(np.float32)

        return rotate(q), rotate(k)

    def _sliding_window_mask(self, seq: int) -> np.ndarray:
        return self.v4._sliding_mask(seq)

    def _attend(self, q: np.ndarray, k: np.ndarray, v: np.ndarray, mask: np.ndarray) -> np.ndarray:
        return self.v4._attend(q, k, v, mask)

    def _pool_kv(self, k: np.ndarray, v: np.ndarray, rate: int, *, overlap: bool) -> tuple[np.ndarray, np.ndarray]:
        pooled_k, pooled_v, _closes = self.v4._pool_kv(k, v, rate, overlap=overlap)
        return pooled_k, pooled_v

    def _sliding_attention(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self.v4._sliding(hidden, layer_index, inference=inference)

    def _csa_attention(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self.v4._csa(hidden, layer_index, inference=inference)

    def _hca_attention(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self.v4._hca(hidden, layer_index, inference=inference)
    ...
    def _ffn_expert(self, flat: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        ...
        limit = 10.0
        gate = pre / (1.0 + np.exp(-np.clip(pre, -12.0, 12.0)))
        up = np.clip(pre, -limit, limit)
        h = (gate * up).astype(np.float32)
        return self._bitlinear(h, layer_index, 5, inference=inference)

    @staticmethod
    def _sqrt_softplus(values: np.ndarray) -> np.ndarray:
        clipped = np.clip(values, -20.0, 20.0)
        return np.sqrt(np.log1p(np.exp(clipped)) + BITNET_EPS).astype(np.float32)

    def _ffn(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self.v4.ffn(hidden, layer_index, inference=inference)

    @staticmethod
    def _sinkhorn(matrix: np.ndarray, iters: int = 8, eps: float = 1e-6) -> np.ndarray:
        return DeepSeekV4FlashBackend.sinkhorn(matrix, iters=iters, eps=eps)

    def _mhc(self, residual: np.ndarray, sublayer: np.ndarray, which: str, layer_index: int) -> np.ndarray:
        return self.v4.mhc(residual, sublayer, which, layer_index)

    def _mhc_depth_combine(self, snapshots: list[np.ndarray]) -> np.ndarray:
        return self.v4.mtp_combine(snapshots)

    def _transformer_block(self, hidden: np.ndarray, layer_index: int, *, inference: bool) -> np.ndarray:
        return self.v4.block(hidden, layer_index, inference=inference)
    ...
    def _forward_hidden(
        ...
        *,
        ...
        self._fwd_token_ids = [int(v) for v in np.asarray(contexts)[0].tolist()]
        ...
        # Inference: Mixture-of-Depths capacity selection (V4 Flash interactivity).
        layers = (
        ...
        for layer_index in layers:
            ...
            mixed_tail = self._mhc_depth_combine(snapshots + [hidden[:, -1, :].copy()])
            hidden = hidden.copy()
            hidden[:, -1, :] = (
    ...
    def next_token_probabilities(self, token_ids: list[int]) -> np.ndarray:
        # Real BitNet decode: full DeepSeek V4 Flash Transformer (CSA/HCA + MoE BitLinear).
        _, _, probabilities = self._transformer_logits(
    ...
    def model_card(self) -> dict[str, object]:
        ...
        return {
            ...
            "model_id": MODEL_ID,
            "version": APP_VERSION,
            "kind": "experimental BitNet b1.58 W1.58A8 DeepSeek V4 Flash CSA/HCA/mHC runtime with a ~10B-slot sparse bank",
            "files": FILES_MODE,
            "only_program": SCRIPT_NAME,
            "generates_program_files": False,
            "engine": ENGINE_ID,
            ...
            "csa_compress": self.config.csa_compress,
            "hca_compress": self.config.hca_compress,
            "o_groups": self.config.o_groups,
            "n_kv_heads": self.config.n_kv_heads,
            "moe_top_k": self.config.moe_top_k,
            "mhc_enabled": self.config.mhc_enabled,
            "flash_attn_calls": self.flash_attn_calls,
            "mhc_mixes": self.mhc_mixes,
            "moe_routes": self.moe_routes,
            "hash_moe_hits": self.hash_moe_hits,
            "deepseek_v4_backend_present": isinstance(getattr(self, "v4", None), DeepSeekV4FlashBackend),
            "hash_moe_tid2eid_in_ram": (
                isinstance(getattr(getattr(self, "v4", None), "tid2eid", None), np.ndarray)
                and int(self.v4.tid2eid.size) == int(self.tokenizer.vocab_size)
            ),
            "tid2eid_ram_slots": int(getattr(getattr(self, "v4", None), "tid2eid", np.zeros(0)).size),
            "v4_backend": self.v4.status() if getattr(self, "v4", None) is not None else None,
            ...
            "reasoning_equation": "DeepSeek V4 Flash backend: concat CSA/HCA KV, attention sinks, mHC streams, Hash-MoE tid2eid",
            ...
            "normal_chat_dispatch": "CatSeekR110B.reply hybrid router",
            ...
            "deepseek_v4_flash_engine": True,
            "deepseek_v4_backend_class": "DeepSeekV4FlashBackend",
            ...
            "real_parameter_target": 10_000_000_000,
            "honesty": (
                "A ~10B-slot BitNet b1.58 bank is held as sparse ternary pages in RAM "
                ...
                "kernels at inference. DeepSeekV4FlashBackend (files=off) runs CSA/HCA with "
                "concatenated sliding+compressor KV, per-head attention sinks, grouped o_proj, "
                "mHC streams, and a RAM-synthesized Hash-MoE tid2eid table — not a V4 checkpoint. "
                "DSpark adds Markov-draft speculative decode (files=off). "
                "DeepSeek R1-style reasoning adds RAM-only think/answer chains with a local reward head. "
                "files=off also means no generated programs: Write/Edit/Bash and Build file emit are blocked; "
                f"the only program is {SCRIPT_NAME}. "
                "This is an educational local runtime, not a pretrained frontier checkpoint or "
                "DeepSeek-hosted V4-Flash API model — but it is not an FFN-only toy head."
            ),
        }
    ...
    def self_test(self) -> dict[str, object]:
        ...
        concat_ok = False
        sinkhorn_ok = False
        if isinstance(v4, DeepSeekV4FlashBackend):
            try:
                probe = np.zeros((1, 8, self.config.d_model), dtype=np.float32)
                q, k, v, _b, seq, _d = v4._project_qkv(self.rms_norm(probe), 0, inference=True)
                pool_k, _pool_v, closes = v4._pool_kv(k, v, self.config.csa_compress, overlap=True)
                mask = v4._concat_mask(seq, pool_k.shape[2], "csa", q, pool_k, closes)
                concat_ok = mask.shape == (seq, seq + int(pool_k.shape[2]))
                mix = DeepSeekV4FlashBackend.sinkhorn(v4.mhc_attn[0])
                sinkhorn_ok = bool(
                    np.allclose(mix.sum(axis=1), 1.0, atol=1e-3)
                    and np.allclose(mix.sum(axis=0), 1.0, atol=1e-3)
                )
            except Exception:
                concat_ok = False
                sinkhorn_ok = False
        tests = {
            ...
            "parameter_count_is_about_10b": 9_500_000_000 <= params <= 10_600_000_000,
            ...
            "no_program_files_emitted": FILES_MODE == "off",
            "only_program_is_this_py": SCRIPT_NAME == "catsseekr1.xx.py",
            "no_checkpoint_or_network": self.external_load_count == 0,
            "engine_is_deepseek_v4_flash": self.config.engine == ENGINE_ID and ENGINE_ID == "deepseek-v4-flash",
            "deepseek_v4_backend_present": isinstance(v4, DeepSeekV4FlashBackend),
            "hash_moe_tid2eid_in_ram": (
                isinstance(v4, DeepSeekV4FlashBackend)
                and isinstance(v4.tid2eid, np.ndarray)
                and int(v4.tid2eid.size) == int(self.tokenizer.vocab_size)
                and v4.tid2eid.dtype == np.int32
            ),
            "deepseek_v4_concat_attn_mask": concat_ok,
            "deepseek_v4_sinkhorn_doubly_stochastic": sinkhorn_ok,
            "deepseek_v4_flash_attn_ran": self.flash_attn_calls > 0 or self.last_engine_path.split("+")[0] in {
                "csa", "hca", "sliding", "sliding-train",
            },
            "ste_shadow_parameters_present": bool(self.shadow) and all(
                name in self.shadow[0] for name in self.MATRIX_NAMES
            ),
            "deepseek_v4_mhc_ran": (not self.config.mhc_enabled) or self.mhc_mixes > 0,
            "deepseek_v4_moe_ran": self.moe_routes > 0 or self.config.moe_top_k <= 1,
...
class OpenDomainResponder:
    """Prompt-grounded heuristics that answer English, Mandarin, and code.

    Specialized intents still transform payload in the prompt. Anything else
    still gets a non-empty, language-matched reply instead of a refusal.
    """
    ...
    INTENT_PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
        ...
        ("code", re.compile(
            r"```|\b(?:python|javascript|typescript|function|def |class |import |traceback)\b|"
            r"这段代码|写一段代码|帮我改代码",
            re.I,
        )),
    ...
    def analyze(cls, prompt: str) -> PromptAnalysis:
        ...
        for name, pattern in cls.INTENT_PATTERNS:
            if pattern.search(raw):
                intent = name
                break
        if looks_like_code(raw) and intent in {
            "conversation", "question", "explain", "build", "casual", "code",
        }:
            if not re.search(
                r"\b(?:build|make|create)\s+(?:an?\s+)?(?:app|game|website|site|dashboard)\b|"
                r"做一个(?:应用|游戏|网站)",
                raw,
                re.I,
            ):
        ...
        return PromptAnalysis(intent, subject, language, payload, modality)
    ...
    def _build(subject: str, language: str) -> str:
        if language == "zh":
            return (
                f"files=off：不会生成程序文件。唯一的程序是 `{SCRIPT_NAME}`。"
                f"关于「{subject}」请在对话里提问；我不会写入 builds/ 或新脚本。"
            )
        return (
            f"{files_off_no_programs(kind='app')}\n\n"
            f"Goal mentioned: {subject}. Ask in chat; I will not emit an app, game, or script file."
        )
    ...
    def _code_language(text: str) -> str:
        ...
        lowered = text.lower()
        rules = (
            ("python", r"\b(?:def |async def |import |print\(|elif |None\b|self\.)"),
            ("javascript", r"\b(?:const |let |=>|console\.|function )"),
            ("typescript", r"\b(?:interface |type |:\s*(?:string|number|boolean)\b)"),
            ("rust", r"\b(?:fn |let mut |impl |pub struct )"),
            ("go", r"\b(?:func |package |fmt\.)"),
            ("java", r"\b(?:public static void|System\.out)"),
            ("c", r"#include|int main\s*\("),
            ("html", r"<html|</div>|<!DOCTYPE"),
            ("bash", r"^\s*#!/bin/(?:ba)?sh|\becho\s+"),
        )
    ...
    @classmethod
    def _code_source(cls, prompt: str, payload: str) -> str:
        if payload.strip():
        ...
        lines = [
            ...
            if CODE_LINE_RE.search(line) or line.strip().endswith((";", "{", "}", ":"))
        ]
        return "\n".join(lines).strip()

    @classmethod
    def _respond_code(cls, prompt: str, analysis: PromptAnalysis) -> str:
        source = cls._code_source(prompt, analysis.payload)
        lang = cls._code_language(prompt if not source else source)
        zh = analysis.language == "zh" or analysis.modality == "mixed"
        names = re.findall(r"\b(?:def|class|function|fn|func)\s+([A-Za-z_][\w]*)", source)
        headline = ", ".join(names[:6]) if names else (analysis.subject[:80] or lang)
        if source:
            preview = "\n".join(source.splitlines()[:12])
            if zh:
                return (
                    f"这是 **{lang}** 代码（{headline}）。\n\n"
                    "它在做什么：按名称/缩进读控制流，先看输入、再看返回值、再看副作用。\n"
                    "files=off：我不会把这段写成磁盘上的新程序；唯一的程序是 catsseekr1.xx.py。\n\n"
                    f"```{lang}\n{preview}\n```"
                )
            return (
                f"This looks like **{lang}** ({headline}).\n\n"
                "Read it as control flow: inputs → transformation → return/side effects. "
                "files=off: I will not write a new program to disk; the only program is catsseekr1.xx.py.\n\n"
                f"```{lang}\n{preview}\n```"
            )
        if zh:
            return (
                f"files=off：不会生成程序文件。唯一的程序是 `{SCRIPT_NAME}`。"
                f"关于「{analysis.subject}」请在对话里说明；我不会写出新的 {lang} 脚本。"
            )
        return (
            f"{files_off_no_programs(kind='script')}\n\n"
            f"Subject: {analysis.subject}. Paste code to discuss it in chat; I will not emit a {lang} file."
        )

    @classmethod
    def _respond_freeform(cls, prompt: str, analysis: PromptAnalysis) -> str:
        """Always answer English or Mandarin using the prompt as grounding."""
        subject = analysis.subject
        zh = analysis.language == "zh"
        body = analysis.payload or prompt.strip()
        sentences = cls._sentences(body)
        gist = sentences[0] if sentences else subject
        if analysis.intent in {"explain", "question"}:
            if zh:
                return (
                    f"问题：{subject}\n\n"
                    f"直接回应：把「{cls._clean(gist, 140)}」拆成目标、已知条件、和一个可验证的下一步。"
                    "若这是事实问题而提示里没有数据，我会标明不确定，但仍然给出查证方法和可用结构，而不是空白拒绝。\n\n"
                    "1. 用你自己的话重述目标。\n"
                    "2. 列出提示里已经出现的事实或约束。\n"
                    "3. 做一个最小检验（例子、反例、或权威来源）。"
                )
            return (
                f"Question: {subject}\n\n"
                f"Direct take: treat “{cls._clean(gist, 140)}” as a goal plus constraints, then pick one checkable next step. "
                "If this needs an external fact that is not in the prompt, I will mark the uncertainty — "
                "and still answer with a method instead of refusing.\n\n"
                "1. Restate the goal in one sentence.\n"
                "2. List facts or limits already in your prompt.\n"
                "3. Run the smallest test (example, counterexample, or an authoritative source)."
            )
        if zh:
            return (
                f"收到：{subject}\n\n"
                f"我会按中文处理这条请求。当前材料是「{cls._clean(gist, 160)}」。\n"
                "下一步可以是：澄清目标、贴出相关代码/原文，或让我按上面的材料给出清单、改写或调试步骤。"
            )
        return (
            f"Got it: {subject}\n\n"
            f"I will answer in English. Working material: “{cls._clean(gist, 160)}”.\n"
            "A concrete next step: clarify the outcome you want, paste related code or source text, "
            "or ask me to turn the material above into a checklist, rewrite, or debug plan."
        )
    ...
    def respond(cls, prompt: str) -> ReasoningAnswer:
        ...
        if analysis.intent == "code" or analysis.modality in {"code", "mixed"}:
            return ReasoningAnswer(cls._respond_code(raw, analysis), "open-domain:code")
        ...
        text = cls._respond_freeform(raw, analysis)
        route = f"open-domain:{analysis.intent}"
        if analysis.intent not in {"explain", "question", "casual", "conversation"}:
            route = "open-domain:conversation"
        return ReasoningAnswer(text, route)
    ...
    def self_test(self) -> dict[str, object]:
        ...
        checks["code_reply_uses_source"] = "add" in outputs.get("code", "").lower()
        checks["mandarin_reply_is_zh"] = bool(HAN_RE.search(outputs.get("mandarin", "")))
        checks["english_reply_is_en"] = bool(LATIN_RE.search(outputs.get("explain", ""))) and not (
            len(HAN_RE.findall(outputs.get("explain", ""))) >= 2
        )
        return {
...
class CatSeekR110BCodeTools:
    """Inspect-only tools when files=off (no Write/Edit/Bash program generation)."""
    ...
    @staticmethod
    def _blocked(name: str) -> ToolResult:
        return ToolResult(name, False, files_off_no_programs(kind="program"))
    ...
    def bash(self, command: str, timeout_s: float = 20.0) -> ToolResult:
        return self._blocked("Bash")


class CatSeekR110BCodeAgent:
    """Local inspect-only agent (files=off: no generated programs)."""

    def __init__(self, engine: "CatSeekR110B", root: Optional[str] = None):
        self.engine = engine
        self.tools = CatSeekR110BCodeTools(root)
    ...
    def run(self, prompt: str) -> Reply:
        ...
        if actions:
            ...
            footer = (
                f"\n\ncatseekr1-10b Code · {ok_n}/{len(results)} tools ok · "
                f"brain {MODEL_ID} · model files={FILES_MODE}"
            )
            text = body + footer
            route = f"catseekr1-10b-code:{len(actions)}tools"
        else:
            # No tool plan: fall through the full catseekr1-10b stack (exact/semantic/BitNet).
            neural = self.engine.reply(prompt)
            text = neural.text
            route = f"catseekr1-10b-code:{neural.route}"
    ...
    def self_test(self) -> dict[str, object]:
        probe = self.tools.ls(".")
        py_glob = self.tools.glob("*.py")
        denied = self.tools.write("generated_app.py", "print(1)\n")
        return {
            "ok": bool(probe.ok and py_glob.ok and (not denied.ok) and FILES_MODE == "off"),
            "workspace": str(self.tools.root),
            "ls_ok": probe.ok,
            "glob_py_ok": py_glob.ok,
            "write_blocked": not denied.ok,
            "tools": ["LS", "Read", "Glob", "Grep"],
            "model_files_mode": FILES_MODE,
            "no_program_artifacts": True,
            "only_program": SCRIPT_NAME,
            "brain": MODEL_ID,
        }


class CatSeekR110BBuildMode:
    """files=off: describe ideas in chat; never write app/game files (only this .py)."""
    ...
    BUILD_RE = re.compile(
        ...
        r"|/build\b|grok\s*build|catseekr1-10b\s*build|catseek\s*build",
        re.IGNORECASE,
    )
...
<title>{title} · catseekr1-10b Build</title>
<style>
...
<header>
  <h1>{title}</h1>
  <p>catseekr1-10b Build · local HTML artifact · BitNet brain files=off</p>
</header>
<main>
{body}
</main>
...
"""
            ...
            script = r"""
const list=document.getElementById('list'),items=JSON.parse(localStorage.getItem('catseekr1-10b-todo')||'[]');
function save(){localStorage.setItem('catseekr1-10b-todo',JSON.stringify(items));}
...
"""
            ...
            body = f'<div class="card"><h2>Ship the idea</h2><p class="muted">Prompt: {prompt[:180].replace("<","&lt;")}</p><p>This landing page was generated by catseekr1-10b Build from your description. Replace the copy, drop in art, and publish anywhere static HTML hosts.</p><div class="row"><a class="btn" href="#get">Get started</a><button onclick="alert(\'Local catseekr1-10b Build CTA\')">Live demo</button></div></div><div class="card" id="get" style="margin-top:14px"><h3>Why catseekr1-10b Build</h3><ul><li>Plain-language → working HTML</li><li>BitNet brain stays files=off</li><li>Iterate with /build again</li></ul></div>'
            script = "/* static landing */"
        else:
            body = f'<div class="card"><h2>Interactive canvas</h2><p class="muted">{prompt[:200].replace("<","&lt;")}</p><canvas id="c" width="480" height="320"></canvas><p class="muted">Click to paint · generated by catseekr1-10b Build</p></div>'
            script = r"""
...
"""
    ...
    def build(self, prompt: str) -> Reply:
        ...
        self.last_path = None
        text = (
            f"{files_off_no_programs(kind=kind)}\n\n"
            f"Request classified as **{kind}**. I will not save HTML/JS or any other program. "
            f"The BitNet, GUI, and replies already live in `{SCRIPT_NAME}`."
        )
        return Reply(
            text=text,
            route=f"catseekr1-10b-build:{kind}",
            elapsed_ms=(time.perf_counter() - started) * 1000.0,
    ...
    def self_test(self) -> dict[str, object]:
        ...
        wrote = bool(self.last_path is not None)
        dir_made = self.out_dir.exists()
        return {
            "ok": (
                (not wrote)
                and (not dir_made)
                and FILES_MODE == "off"
                and reply.route.startswith("catseekr1-10b-build:")
                and "files=off" in reply.text.lower()
            ),
            "route": reply.route,
            "wants_snake": self.wants("make a snake game"),
            "wants_todo": self.wants("build a todo app"),
            "wrote_file": wrote,
            "builds_dir_created": dir_made,
            "only_program": SCRIPT_NAME,
        }
...
class CatSeekR110B:
    ...
    def __init__(
        ...
        self.code_agent = CatSeekR110BCodeAgent(self)
        self.build_mode = CatSeekR110BBuildMode()
    ...
    def model_card(self) -> dict[str, object]:
        ...
        card.update({
            "runtime": "catseekr1-10b cognitive inference runtime",
            "reasoning_tools": [
                ...
                "catseekr1-10b Code inspect-only (files=off: no Write/Edit/Bash)",
            ],
    ...
    def self_test(self) -> dict[str, object]:
        ...
        return {
            ...
            "catseekr1_10b_code": code,
            "catseekr1_10b_build": build,
            "runtime": {
                "files": FILES_MODE,
                "network_required": False,
                "boot_elapsed_s": round(getattr(self, "boot_elapsed_s", 0.0), 4),
                ...
                "deepseek_v4_backend_present": isinstance(getattr(self.model, "v4", None), DeepSeekV4FlashBackend),
                "hash_moe_tid2eid_in_ram": (
                    isinstance(getattr(getattr(self.model, "v4", None), "tid2eid", None), np.ndarray)
                    and int(self.model.v4.tid2eid.size) == int(self.model.tokenizer.vocab_size)
                ),
    ...
    def _command(self, prompt: str) -> Optional[Reply]:
        ...
        if lower in {"/help", "/?"}:
            text = (
                ...
                "- `/code PROMPT` — inspect-only (files=off: no new program files)\n"
                "- `/build PROMPT` — describe an idea in chat; does not write apps/games\n"
                "- Complex prompts auto-route through DeepSeek R1-style think/answer on BitNet + DSpark (files=off)\n\n"
                "Ordinary messages stay inside this .py. files=off: no checkpoints, no generated programs. The ~10B value describes packed architecture slots, not pretrained frontier knowledge."
            )
        ...
        if lower == "/trace":
            if not self.model.last_trace:
                text = "No catseekr1-10b generation trace exists yet."
            else:
                rows = [
                    {
                        "step": step.index,
        ...
        if lower == "/clear":
            self.clear()
            return Reply("catseekr1-10b conversation context cleared.", "control:/clear", (time.perf_counter() - started) * 1000)
        if lower.startswith("/train"):
            ...
            text = (
                f"catseekr1-10b trained for {report.steps} additional RAM-only steps. "
        ...
        if lower.startswith("/candidates"):
            parts = raw.split()
            if len(parts) == 1:
                text = f"catseekr1-10b uses up to {self.candidate_count} verified candidates."
            else:
                try:
                    value = int(parts[1])
                except ValueError:
                ...
                text = f"catseekr1-10b candidate count set to {self.candidate_count}."
            return Reply(text, "control:/candidates", (time.perf_counter() - started) * 1000)
        if lower.startswith("/temperature"):
            parts = raw.split()
            if len(parts) == 1:
                text = f"catseekr1-10b temperature is {self.temperature:.2f}."
            else:
                try:
                    value = float(parts[1])
                except ValueError:
                ...
                text = f"catseekr1-10b temperature set to {self.temperature:.2f}."
            return Reply(text, "control:/temperature", (time.perf_counter() - started) * 1000)
        if lower.startswith("/code"):
            task = raw[len("/code"):].strip()
            if not task:
                return Reply(
                    "Usage: /code PROMPT   (inspect-only; files=off writes nothing)",
                    "control:/code",
                    (time.perf_counter() - started) * 1000,
                )
        ...
        if lower.startswith("/build"):
            ...
            if not task:
                return Reply(
                    "Usage: /build PROMPT   e.g. /build snake game\nfiles=off: no HTML/app file is written.",
                    "control:/build",
                    (time.perf_counter() - started) * 1000,
                )
            reply = self.build_mode.build(task)
            return Reply(reply.text, reply.route, (time.perf_counter() - started) * 1000.0, reply.tokens, reply.tokens_per_second)
        return Reply("Unknown catseekr1-10b command. Use /help.", "control:unknown", (time.perf_counter() - started) * 1000)
    ...
    def reply(self, prompt: str, on_token: Optional[Callable[[str], None]] = None) -> Reply:
        ...
        if CatSeekR110BBuildMode.wants(prompt):
        ...
        if analysis.intent in {
            ...
            "build", "brainstorm", "current", "verify", "code",
            "explain", "question", "casual",
        } or analysis.modality in {"code", "mixed"}:
        ...
        if self.model.training_steps <= 0:
            grounded = self.open_domain.respond(prompt)
            self.history.append((prompt, grounded.text))
            self.history = self.history[-12:]
            if on_token:
...
class CatSeekR110BGUI:
    ...
    def __init__(self, root: tk.Tk, engine: CatSeekR110B):
        self.root = root
        self.engine = engine
    ...
    def _build(self) -> None:
        ...
        base = tk.Menu(menu, tearoff=False, bg=self.PANEL, fg=self.TEXT)
        base.add_command(label="New catseekr1-10b chat", command=self._new_chat)
        base.add_command(label="catseekr1-10b model card", command=lambda: self._submit("/model"))
        base.add_command(label="catseekr1-10b inference self-test", command=lambda: self._submit("/selftest"))
        base.add_separator()
        base.add_command(label="Exit catseekr1-10b", command=root.destroy)
        menu.add_cascade(label=APP_NAME, menu=base)
        ...
        tk.Label(
            name,
            text=(
                f"Real BitNet b1.58 ~10B · {ENGINE_NAME} · {self.engine.model.parameter_count:,} params · "
        ...
        self.status = tk.Label(header, text="catseekr1-10b inference ready", bg=self.PANEL, fg=self.BLUE_2, font=self._font(10))
        ...
        for line in (
            ...
            "● Next-token softmax",
            "● Real BitNet b1.58 ~10B W1.58A8",
            f"● {ENGINE_NAME} engine (CSA/HCA/mHC/MoE)",
            "● DSpark speculative decode",
            "● DeepSeek R1 think/answer (RAM)",
            "● vibe-check corpus + /vibecheck",
    ...
    def _new_chat(self) -> None:
        ...
        if self.busy:
            ...
            self.status.config(text="catseekr1-10b new chat ready")
        self.entry.focus_set()
    ...
    def _submit(self, forced: Optional[str] = None) -> None:
        ...
        self.status.config(text="catseekr1-10b computing next tokens…")
        ...
        try:
            while True:
                ...
                if kind == "done" and is_current:
                    ...
                    self.status.config(text=f"catseekr1-10b ready · {reply.tokens_per_second:.1f} tok/s")
                elif kind == "error" and is_current:
                    self._append("ERROR\n" + str(payload) + "\n", "error")
                    self.status.config(text="catseekr1-10b error")
...
def make_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--code", action="store_true", help="inspect-only code REPL (files=off: no program files)")
    parser.add_argument("--build", action="store_true", help="chat about app/game ideas (files=off: writes nothing)")
    parser.add_argument("--cwd", default=None, help="workspace root for inspect-only LS/Read/Glob/Grep")
    parser.add_argument("--prompt", help="generate one answer and exit")
    parser.add_argument(
        "--vibe-check",
        action="store_true",
        help="audit embedded training data and arbitrary-prompt coverage without allocating the 10B bank",
    )
...
def terminal(engine: CatSeekR110B, one_prompt: Optional[str] = None) -> int:
...
def code_terminal(engine: CatSeekR110B, one_prompt: Optional[str] = None, cwd: Optional[str] = None) -> int:
    agent = CatSeekR110BCodeAgent(engine, root=cwd)
    engine.code_agent = agent
    banner = (
        f"{APP_NAME} Code · inspect-only · brain {MODEL_ID} · files={FILES_MODE}\n"
        f"workspace: {agent.tools.root}\n"
        f"only program: {SCRIPT_NAME} · tools: LS Read Glob Grep · Write/Edit/Bash blocked"
    )
    print(banner)
...
def build_terminal(engine: CatSeekR110B, one_prompt: Optional[str] = None, cwd: Optional[str] = None) -> int:
    builder = CatSeekR110BBuildMode(root=cwd)
    engine.build_mode = builder
    banner = (
        f"{APP_NAME} Build · files={FILES_MODE} · no artifacts · only `{SCRIPT_NAME}`\n"
        "describe an idea in chat · nothing is written to disk · /quit to exit"
    )
    print(banner)
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    profiles = {
        # All profiles keep the real ~10B BitNet + DeepSeek V4 Flash trunk; default boot ≤0.2s.
        "fable5": dict(
            context_tokens=24,
            mod_capacity=2,
            train_layers=1,
    ...
    config = ModelConfig(
        ...
        embedding_dim=BITNET_10B_D_MODEL,
        hidden_dim=BITNET_10B_D_FF,
        d_model=BITNET_10B_D_MODEL,
        d_ff=BITNET_10B_D_FF,
        n_layers=BITNET_10B_N_LAYERS,
        n_heads=BITNET_10B_N_HEADS,
        mod_capacity=profile["mod_capacity"],
        train_layers=profile["train_layers"],
        dense_init=False,
        reasoning_passes=profile["mod_capacity"],
        max_reasoning_passes=BITNET_10B_N_LAYERS,
        ...
        engine=ENGINE_ID,
        csa_compress=DEEPSEEK_V4_CSA_COMPRESS,
        hca_compress=DEEPSEEK_V4_HCA_COMPRESS,
        sliding_window=DEEPSEEK_V4_SLIDING_WINDOW,
        index_topk=DEEPSEEK_V4_INDEX_TOPK,
        hc_mult=DEEPSEEK_V4_HC_MULT,
        hash_moe_layers=DEEPSEEK_V4_HASH_MOE_LAYERS,
        moe_top_k=DEEPSEEK_MOE_TOP_K,
        mhc_enabled=True,
    ...
    if show_startup_progress:
        print(
            f"Materializing ~10B BitNet b1.58 + {ENGINE_NAME} in RAM (files=off, {config.speed_target}): "
    ...
    try:
        engine = CatSeekR110B(config, progress=progress)
    ...
    if not (args.self_test or args.model_card):
        print(
            ...
            f"files={FILES_MODE} · BitNet b1.58 ~10B slots",
            file=sys.stderr,
            flush=True,
        )
    if args.self_test:
